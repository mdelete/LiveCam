...
- (void)encodeAudioData:(nullable NSData*)audioData timeStamp:(uint64_t)timeStamp {
    ...
    if(leftLength + audioData.length >= self.configuration.bufferLength){
        ...
        memset(totalBuf, 0, (int)totalSize); // FIXED: parameter seemed to be swapped
...
- (BOOL)createAudioConvert { //根据输入样本初始化一个编码转换器
    ...
    if(result == noErr) {
        AudioConverterSetProperty(m_converter, kAudioConverterEncodeBitRate, propSize, &outputBitrate);
    }
